#define LOG_MODULE "bio2drv-bi2a_drs"
...
static const uint8_t _BIO2DR_BI2A_DRS_INIT_DATA = 0x2D;

static bool
bio2drv_bi2a_drs_init_io(struct aciodrv_device_ctx *device, uint8_t node_id)
{
    ...
    msg.cmd.nbytes = 1;
    msg.cmd.param = _BIO2DR_BI2A_DRS_INIT_DATA;
...
static bool bio2drv_bi2a_drs_watchdog_start(
...
bool bio2drv_bi2a_drs_init(struct aciodrv_device_ctx *device, uint8_t node_id)
{
    ...
    if (!bio2drv_bi2a_drs_init_io(device, node_id)) {
        return false;
    }

    if (!bio2drv_bi2a_drs_watchdog_start(device, node_id)) {
        return false;
    }
    ...
    struct bi2a_drs_state_in *pin)
{
    ...
    *(struct bi2a_drs_state_out *) msg.cmd.raw = *pout;
